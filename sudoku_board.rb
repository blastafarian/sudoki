...
class SudokuBoard
  ...
  def placeValue(row, column, placedValue)
  ...
  # From the set of cells that don't have a placed value, returns the details
  # (row number, column number and array of candidate values) of a cell that
  # has the minimum number of candidate values.
  # 
  # If there are multiple such cells, returns the details of any one of them.
  #
  # If there are no such cells, returns nil, nil and nil.
  def findCellWithLeastCandidateValues()
    minRow, minColumn, minCandidateValues = nil, nil, nil
    ROWS.each do |row|
      COLUMNS.each do |column|
        cell = getCell(row, column)
        if getCell(row, column).getPlacedValue() == nil
  ...
  def validRow?(row)
    ...
    (! arrayHasDuplicates?(valuesOfRow)) &&
      valuesOfRow.to_set().subset?(VALID_CELL_VALUES)
  end
  ...
  def validColumn?(column)
    ...
    (! arrayHasDuplicates?(valuesOfColumn)) &&
      valuesOfColumn.to_set().subset?(VALID_CELL_VALUES)
  end
  ...
  def validSquare?(square)
    ...
    (! arrayHasDuplicates?(valuesOfSquare)) &&
      valuesOfSquare.to_set().subset?(VALID_CELL_VALUES)
  end
